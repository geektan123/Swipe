...
public class AddProductBottomSheet extends BottomSheetDialogFragment {
    ...
    private void validateAndSubmit() {
        ...
        if (isInternetAvailable(getContext())) {
            ...
            saveProductOffline(name, priceValue, taxValue, type, selectedImageUri.toString());
        }
    }

    private void saveProductOffline(String name, double price, double tax, String type, String imageUri) {
// Assuming you need to pass two boolean values (e.g., 'isAvailable' and 'isSyncPending')
        Product product = new Product(name, price, tax, type, imageUri, false, false);
        new Thread(() -> {
            ProductDatabase.getInstance(getContext()).productDao().insert(product);
